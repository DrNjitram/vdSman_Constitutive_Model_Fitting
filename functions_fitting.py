...
def read_data_CCR_cycle(path_read, idx, prefix=""):
    idx = ".asc" if not idx else str(idx)
    filepaths = []
    for path, subdirs, files in os.walk(path_read):
        for file in files:
            if file.startswith(prefix) and file.endswith(idx):
                filepaths.append(os.path.join(path, file))
    data = []
    info = []
    for filepath in filepaths:
        info.append(os.path.basename(filepath)[len(prefix):len(os.path.basename(filepath))-len(idx) -1].replace("_", "."))

        data.append(np.loadtxt(filepath, delimiter=";"))
    return data, info
...
def extract_steady_from_laos(data_exp_avg, nelements_cycle, col_index_shearrate, col_index_stress):
    ...
    for k in range(0, len(data_exp_avg)):
        ...
        x = data_exp_avg[k][:, col_index_shearrate][
            -nelements_cycle:]  #shear rate [1/s] -Martijn: Strain rate gamma dot [1/s]
        ...
        for i in np.linspace(minindex_x, 0, minindex_x + 1):
            i = int(i)
            if ypos[i - 1] < ypos[i] and ypos[i - 2] < ypos[i]:
                break
            else:
                ydat.append(ypos[i]); xdat.append(xpos[i])
...
def Ruud_Single_Cycle_Oldroydb(strain_lin, freq, params, ncycles, ntimesteps_percycle, gammadot_data):
    ...
    for cycle in range(0, ncycles):
        ...
        sum_G1 = 0
        sum_G2 = 0
        # t in range (0,ntimesteps_percycle):
        for t, gammadot in enumerate(gammadot_data):
...
def optimization_lin_perfreq_ruud(initial_guess_lin, bounds_lin_norm, strain_val_lin,
                                  ...
                                  nelements_cycle, col_index_shear):
    def objective_linear(params):
        ...
        #extract the data
        last2cycles = all_data[0][0][col_index_stress][
                      index * nelements_cycle:(index + 1) * nelements_cycle]  #waveform data at given frequency
        gammadotlist = all_data[0][0][col_index_shear][
                       index * nelements_cycle:(index + 1) * nelements_cycle]  #gammadot for inputtin into simulation
...
def optimization_nonlin_together_ruud_HB_given(initial_guess_nonlin, HBparam, bounds_nonlin_norm,
                                               ...
                                               col_index_stress, col_index_shearrate, all_data, optimized_params_lin,
                                               nelements_cycle):
    def objective_maxnorm(params):
        ...
        for freqvals in freq:
            ...
            for strainval in strain_values_nonlin:
                ...
                gammadot_data = all_data[strain_index + len(strain_values_nonlin) * freqindex][col_index_shearrate][
                                -nelements_cycle:]
    ...
    result = minimize(fun=objective_maxnorm, x0=initial_guess_nonlin,
...
def sum_of_squared_residuals_ruud(params_actual, current_freq, strain_val_lin, \
    ...
    last2cycles = all_data[0][0][col_index_stress][
                  index * nelements_cycle:(index + 1) * nelements_cycle]  #waveform data at given frequency
    gammadotlist = all_data[0][0][col_index_shearrate][
                   index * nelements_cycle:(index + 1) * nelements_cycle]  #gammadot for inputtin into simulation
    for elem in last2cycles:
        extracted_ydata.append(elem)
    maxelem = max(last2cycles)
...
def sum_of_squared_residuals_laos_ruud(params_actual, strain_values_nonlin, freq, cycles, eta_inf, ntimesteps_cycle, \
                                       optimized_params_lin, col_index_stress, col_index_shearrate, all_data,
                                       nelements_cycle):
    #Extract predicted data
    #doing everything per frequency
    pred_extracted_ydata = []  #all data will be here

    for freqindex, freqvals in enumerate(freq):
        modulus = optimized_params_lin[0][0] * freqvals ** optimized_params_lin[0][1]
        t_rel = optimized_params_lin[1][0] * freqvals ** optimized_params_lin[1][1]


        for strain_index, strainval in enumerate(strain_values_nonlin):
