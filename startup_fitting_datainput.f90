...
program startup
  ...
  integer, parameter :: model      = 2, &  ! 2: Oldroyd model 3: Giesekus
                        ...
                        alam_model = 4, &  ! adapted lambda model, 4=ruud's
  ...
  real(dp) :: c(1,ncompc,nmodes), rhs(1,ncompc,nmodes), tau(1,ncompt), gammadot, timestep, sumG1, sumG2, G1, G2,&
              G_MOD, Lxx, Lyy, Lxy,Lyx, Ltt, Dxx, Dxy, Dyy, eta_eff, gamma_eff, cxx, cxy, cyy, ctt, I1, gammadotlist(257)
  ...
  namelist /comppar/ modulus, lambda, tau_y, nexp, &
                     alpha, gammadot_cr, eta_inf,I1c, &
                     gamma_0,  freq, cycles, filename, numtimesteps_percycle, gammadotlist
...
!  print *, "this is vemodel alam", vemodel%alam
...
! set material parameters
  ...
  vemodel%alam(1:6,1) = [ tau_y, nexp, alpha, gammadot_cr, eta_inf, I1c ]
...
! stepping using RK2
  ...
  do elemcycle = 1, cycles !for each cycle start a loop
    ...
    if ( elemcycle == 1) then
      ...
      step = 0
      write(13,'(1i12.4, 11es12.4)') elemcycle, step * timestep, real(0.00_dp),real(0.00_dp),real(0.00_dp),&
    ...
    ! initial condition for the first step of remaining cycles
      ...
      write(13,'(1i12.4, 11es12.4)') elemcycle, step * timestep, tau, vonmises_2D(tau(1,:)),gammadot,&
      gamma_eff, eta_eff, G_MOD
  ...
  !   from valocity gradient calcuate gamma_eff and eta_eff
      ...
      gamma_eff = sqrt( 2. * ( Dxx*Dxx + Dyy*Dyy + 2*(Dxy**2)) ) 
      eta_eff = tau_y/gamma_eff * (1+ (gamma_eff/gammadot_cr)**nexp ) + eta_inf
  ...
  !   from initial (non updated) conformation tensor calcuate G
      ...
      I1 = cxx + cyy -2
      G_MOD = modulus / (1+abs((I1/I1c))**alpha) !not sure: put an additional abs val.
  ...
  !   compute stress tensor
      ...
      ! sumG1 = sumG1 + tau(1,2) * sin(freq*step*timestep*2*3.14)  
      ! sumG2 = sumG2 + tau(1,2) * cos(freq*step*timestep*2*3.14)
      ...
      write(13,'(1i12.4, 11es12.4)') elemcycle, step * timestep, tau, vonmises_2D(tau(1,:)),gammadot, gamma_eff, eta_eff, G_MOD, I1
    ...
    open ( unit=10, file='strain_sweep.txt', status = 'old', position = 'append' )
